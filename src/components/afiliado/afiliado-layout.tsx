...
import { LayoutDashboard, LogOut, Menu, X, ShoppingBag, ExternalLink } from "lucide-react"
...
export function AfiliadoLayout({ children, userName, userEmail, userImage }: AfiliadoLayoutProps) {
  const pathname = usePathname()
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)

  const navigation = [
    { name: "Panel", href: "/afiliado", icon: LayoutDashboard },
  ]
  ...
  return (
    <div className="min-h-screen bg-zinc-50">
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-zinc-200 flex items-center justify-between px-4 z-50">
        <Link href="/afiliado" className="flex items-center">
          <img
        ...
        <div className="px-6 py-6 border-b border-zinc-200 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center overflow-hidden shrink-0">
              {userImage ? (
                <img src={userImage} alt={userName || "Afiliado"} className="w-full h-full object-cover" />
              ) : (
          ...
          <div className="mt-3 inline-flex items-center gap-1.5 bg-primary/10 rounded-lg px-3 py-1.5">
            ...
            <span className="text-xs font-semibold text-primary">Afiliado</span>
          </div>
        </div>
